...
public class UpdateByExampleWithoutBLOBsMethodGenerator extends AbstractJavaMapperMethodGenerator {
	...
	public void addInterfaceElements(Interface interfaze) {
		...
		method.addParameter(new Parameter(parameterType, getObjectName(parameterType.getShortName()), "@Param(\"record\")")); //$NON-NLS-1$ //$NON-NLS-2$
		importedTypes.add(parameterType);

		FullyQualifiedJavaType exampleType = new FullyQualifiedJavaType(introspectedTable.getExampleType());
		method.addParameter(new Parameter(exampleType, getObjectName(exampleType.getShortName()), "@Param(\"example\")")); //$NON-NLS-1$ //$NON-NLS-2$
